...
在这里，您可以看到所有支持的测试定义，以及如何在 YAML 配置文件中进行配置。

测试定义(Test Definition)是测试的通用定义。测试用例中会指定该测试定义。测试用例是指定测试定义参数的地方。

在本节中，您将了解我们目前支持哪些测试，以及如何在 YAML/JSON 配置文件中进行配置。
...
应用于表顶部的测试。以下是所有表格测试的列表：

- [表行数等于](#table-row-count-to-equal)
- [表行数介于](#table-row-count-to-be-between)
- [表列数等于](#table-column-count-to-equal)
- [表列数介于](#table-column-count-to-be-between)
- [表列名称必须存在](#table-column-name-to-exist)
- [表列匹配集合](#table-column-to-match-set)
- [表自定义SQL测试](#table-custom-sql-test)
- [表行插入数介于两者之间](#table-row-inserted-count-to-be-between)
...
| Condition                                                  | Status    |
|----------------------------------------------------------|---------|
| `value` **match** 表格中的行数          | Success ✅ |
| `value` **does not match** 表格中的行数 | Failed ❌  |
...
- `minValue`： 区间的下限。如果已告知，则行数应大于此值。
- `maxValue`： 区间的上限。如果输入，行数应小于此值。
